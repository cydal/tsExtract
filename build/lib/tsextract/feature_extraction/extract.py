...
def build_features_forecast(data, features_request, include_tzero=True):
    """Performs main feature engineering and returns
       dataframe without target for forecasting
    ...
    -------
    DataFrame
        Pandas df containing features only
    """
    return(_build(data, features_request, target_lag=-1, include_tzero=include_tzero, forecast=True))
...
def build_features(data, features_request, target_lag=3, include_tzero=True):
    ...
    """
    return(_build(data, features_request, target_lag=target_lag, include_tzero=include_tzero, forecast=False))
...
## Build features helper function
def _build(data, features_request, target_lag=3, include_tzero=True, forecast=False):

    features_request = copy.deepcopy(features_request)
    ...
    def window(features_item):
        return(_window_list(data, features_item))

    def window_statistic(features_item):
        return(_win_stat(_window_list(data, features_item[:-1]), features_item[-1]))

    ## Confirm - :-1 --- -1
    def difference(features_item): ## 10 24 10
        return(_difference_comb(data, features_item))

    def momentum(features_item):
        return(_momentum_comb(data, features_item))

    def force(features_item):
        return(_force_comb(data, features_item))

    ### Confirm  1-2 windowing
    def difference_statistic(features_item):
        return(_win_stat(_difference_comb(data, features_item[:-1]), features_item[-1]))

    def momentum_statistic(features_item):
        return(_win_stat(_momentum_comb(data, features_item[:-1]), features_item[-1]))

    def force_statistic(features_item):
        return(_win_stat(_force_comb(data, features_item[:-1]), features_item[-1]))
    ...
    # Get T+ target
    ## -1 means it's a forecast request
    if target_lag != -1:
    ...
    ## Cut off NaNs
    df = _cut_final(df, forecast=forecast)
...
def _difference(data, lag):
  return(data.diff(lag))

def _momentum(data, lag):
  return(_difference(_difference(data, lag), lag))

def _force(data, lag):
  return(_difference(_momentum(data, lag), lag))



## Perform diff/mom/force operations
def _difference_comb(data, window_lag):
  ...
  if len(window_lag) == 2:
    ## Difference Matrix
    multi_diff = _difference(pd.DataFrame(_window_list(data, [window_lag[0]-1])), window_lag[1])
    single_diff = _difference(data, window_lag[1])

    multi_size = multi_diff.shape[1]
    multi_diff = multi_diff.iloc[:, window_lag[1]:]
  else:
    ## Difference Matrix
    multi_diff = _difference(pd.DataFrame(_window_list(data,[window_lag[0]-1 , window_lag[1]-1])), window_lag[2])
    single_diff = _difference(data, window_lag[2])
...
def _momentum_comb(data, window_lag):
  ...
  if len(window_lag) == 2:
    ## Momentum Matrix
    multi_diff = _momentum(pd.DataFrame(_window_list(data, [window_lag[0]-1])), window_lag[1])
    single_diff = _momentum(data, window_lag[1])

    multi_size = multi_diff.shape[1]
    multi_diff = multi_diff.iloc[:, window_lag[1]:]
  else:
    ## Momentum Matrix
    multi_diff = _momentum(pd.DataFrame(_window_list(data,[window_lag[0]-1 , window_lag[1]-1])), window_lag[2])
    single_diff = _momentum(data, window_lag[2])
...
def _force_comb(data, window_lag):
...
## uses num_nan output
def _cut_final(df,forecast=False):
